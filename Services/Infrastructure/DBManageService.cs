...
using Common.Utils;
using DBConstants = Common.DBConstants;
...
namespace Services.Infrastructure
{
    public class DBManageService : IDBManageService
    {
        ...
        public DBManageService(ILoggerFactory logger, IConfiguration config)
        {
            _logger = logger.CreateLogger<DBManageService>();
            _connectionString = config.GetDBSetting("OracleConnectionString") +
                                config.GetDBSetting("OracleConnectionSetting");
        }
        ...
        public bool ExecuteNonQuery(string procedureName, SqlParameter[] parameters, out List<string> outParas)
        {
            ...
            using (OracleCommand cmd = new OracleCommand(procedureName))
            {
                ...
                {
                    _logger.LogError($"ExecuteNonQuery Error: {ex.Message}");
                    return false;
                }
                finally
                {
            ...
            SqlParameter[] parameters)
        {
            ...
            {
                _logger.LogError($"LoadDataTable Error: {ex.Message}");
                return null;
            }
            ...
            out List<string> outParas)
        {
            ...
            try
            {
                using (var cmd = new OracleCommand(procedureName))
                {
                    AddParams(cmd, parameters, false);
                    cmd.Connection = conn;
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        // helper
        protected void AddParams(IDbCommand cmd, SqlParameter[] parameters, bool processSqlInject = true)
        {
            if (processSqlInject)
        ...
        protected void ProcessSqlInjection(string cmdText, SqlParameter[] parameters)
        {
        ...
        protected string GetSqlInjectionStr(SqlParameter[] parameters)
        {
